...
public class PhotoViewActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(getIntent().getExtras()!=null){
            ...
            for (int i = 0; i <t ; i++) {
                PhotoDetails temp=new PhotoDetails((String) getIntent().getExtras().get(BEGIN_TICKKETS_PATH+i),
                                                (String)getIntent().getExtras().get(COUNT_TICKKETS_CACHE+i),
                                                (String)getIntent().getExtras().get(BEGIN_TICKKETS_UID+i));
