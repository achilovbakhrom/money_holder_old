...
public class RecordIncomesView extends View implements 	GestureDetector.OnGestureListener{
	...
	public void updatePageCountAndPosition() {
		this.tableCount = sharedPreferences.getInt("key_for_window_bottom", 4);
		this.currentPage = sharedPreferences.getInt("income_current_page", 0);
		initButtons();
	}
	...
	public void initButtons() {
		...
		List<BoardButton> boardButtonList = boardButtonDao
				.queryBuilder()
				.where(BoardButtonDao.Properties.Table.eq(PocketAccounterGeneral.INCOME))
				.build()
				.list();
		for (int i=0; i<4; i++) {
			switch(i) {
			...
			for (int j=0; j<boardButtonList.size(); j++) {
				if (boardButtonList.get(j).getTable() == PocketAccounterGeneral.INCOME &&
						(boardButtonList.get(j).getPos()-currentPage*PocketAccounterGeneral.INCOME_BUTTONS_COUNT) == i) {
	...
	public void drawPercents() {
		...
		textPaint.setColor(ContextCompat.getColor(getContext(), R.color.green_just));
									...
									warningDialog.setOnYesButtonListener(new OnClickListener() {
										...
										public void onClick(View v) {
											...
											daoSession.getFinanceRecordDao().queryBuilder()
													.where(FinanceRecordDao.Properties.Date.ge(begin),
															FinanceRecordDao.Properties.Date.le(end))
													.buildDelete()
		...
		for (int i=0; i<size; i++) {
			if (buttons.get(i).getContainer().contains(x, y)) {
				...
				List<BoardButton> boardButtonList = daoSession.getBoardButtonDao()
						.queryBuilder().where(BoardButtonDao.Properties.Table.eq(PocketAccounterGeneral.INCOME))
						.list();
	...
	private void openChooseDialogLongPress(final int pos) {
		...
		BoardButton cur = daoSession.getBoardButtonDao().queryBuilder()
				.where(BoardButtonDao.Properties.Pos.eq(pos), BoardButtonDao.Properties.Table.eq(PocketAccounterGeneral.INCOME))
				.list().isEmpty() ?
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				switch (position) {
					case 0:
						...
						break;
					case 1:
						logicManager.changeBoardButton(PocketAccounterGeneral.INCOME, currentPage*PocketAccounterGeneral.INCOME_BUTTONS_COUNT+pos, null);
	...
	private void openCategoryChooseDialog(final int pos) {
		...
		final ArrayList<IconWithName> categories = new ArrayList<>();
		List<RootCategory> categoryList = daoSession.getRootCategoryDao().queryBuilder()
				.where(RootCategoryDao.Properties.Type.eq(PocketAccounterGeneral.INCOME))
				.build()
				.list();
